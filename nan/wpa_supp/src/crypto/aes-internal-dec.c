...
static void
rijndaelDecrypt(const u32 rk[/*44*/], int Nr, const u8 ct[16], u8 pt[16]) {
...
#define ROUND(i, d, s)                                                         \
	do {                                                                   \
		d##0 = TD0(s##0) ^ TD1(s##3) ^ TD2(s##2) ^ TD3(s##1) ^         \
		       rk[4 * i];                                              \
		d##1 = TD0(s##1) ^ TD1(s##0) ^ TD2(s##3) ^ TD3(s##2) ^         \
		       rk[4 * i + 1];                                          \
		d##2 = TD0(s##2) ^ TD1(s##1) ^ TD2(s##0) ^ TD3(s##3) ^         \
		       rk[4 * i + 2];                                          \
		d##3 = TD0(s##3) ^ TD1(s##2) ^ TD2(s##1) ^ TD3(s##0) ^         \
		       rk[4 * i + 3];                                          \
	} while (0)
