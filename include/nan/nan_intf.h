...
enum NAN_BSS_ROLE_INDEX {
	NAN_BSS_INDEX_MAIN = 0,
	NAN_BSS_INDEX_WOSEC = NAN_BSS_INDEX_MAIN,
#if CFG_NAN_BSS_SEPARATE_SEC_ROLE
	NAN_BSS_INDEX_WSEC,
#else
	NAN_BSS_INDEX_WSEC = NAN_BSS_INDEX_WOSEC,
#endif
	NAN_BSS_INDEX_NUM
};
...
struct NanTransmitFollowupRequest {
	...
	enum NanTxPriority priority; /* priority of the request 2=high */
...
struct NanStatsRequest {
	enum NanStatsType stats_type; /* NAN Statistics Request Type */
...
struct NanDiscEngEventInd {
	uint8_t eventID;
	enum NanDiscEngEventType event_type; /* NAN Protocol Event Codes */
	union {
		/*
...
struct NanDataPathInitiatorRequest {
	...
	*/
	uint32_t requestor_instance_id; /* Value 0 for no publish/subscribe */
...
struct NanRangeResponse {
	...
	struct NanRangingCfg ranging_cfg;
	struct NanRangeResponseCtl
		response_ctl; /* NAN Ranging response control parameters*/
} __KAL_ATTRIB_PACKED__;
