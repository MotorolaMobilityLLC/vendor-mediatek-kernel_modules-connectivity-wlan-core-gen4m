...
struct SOC7_0_DMASHDL_CFG rSOC7_0_DmashdlCfg = {
			       ...
			       u_int8_t fgEnable)
{
	...
	if (ucGroup >= ENUM_SOC7_0_DMASHDL_GROUP_NUM)
		ASSERT(0);
...
void soc7_0HalDmashdlSetMinQuota(struct ADAPTER *prAdapter, uint8_t ucGroup,
				 uint16_t u2MinQuota)
{
	uint32_t u4Addr, u4Val;
...
void soc7_0HalDmashdlSetSlotArbiter(struct ADAPTER *prAdapter,
				    u_int8_t fgEnable)
{
	uint32_t u4Val;
...
void soc7_0HalDmashdlSetUserDefinedPriority(struct ADAPTER *prAdapter,
					    uint8_t ucPriority, uint8_t ucGroup)
{
	uint32_t u4Addr, u4Val, u4Mask, u4Shft;

	ASSERT(ucPriority < 16);
	ASSERT(ucGroup < ENUM_SOC7_0_DMASHDL_GROUP_NUM);
