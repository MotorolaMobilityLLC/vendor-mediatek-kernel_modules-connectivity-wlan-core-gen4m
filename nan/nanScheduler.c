					 ...
					 uint8_t ucPriChnlBitmap)
{
	...
	DBGLOG(NAN, INFO,
	       "OC=%u, ChnlBitmap=0x%04x, PriChnlBitmap=%u, fgNonContBw=%u",
...
nanSchedGetFixedChnlInfo(struct ADAPTER *prAdapter) {
	...
	if (prAdapter->rWifiVar.ucNanFixChnl != 0) {
		u4Bw = nanSchedConfigGetAllowedBw(
			prAdapter, prAdapter->rWifiVar.ucNanFixBand);
		rSelChnlInfo = nanRegGenNanChnlInfoByPriChannel(
...
#if (CFG_SUPPORT_NAN_6G == 1)
	uint8_t ucDisc6GChnlBw = MAX_BW_20MHZ;
#endif
...
#if (CFG_SUPPORT_NAN_6G == 1)
	prNanScheduler->fgEn6g = fgEn6g && prAdapter->rWifiVar.ucNanEnable6g;
#endif
...
#if (CFG_SUPPORT_NAN_6G == 1)
	ucDisc6GChnlBw = prAdapter->rWifiVar.ucNan6gBandwidth;
#endif
...
#if (CFG_SUPPORT_NAN_6G == 1)
	if (((u4BandIdMask & BIT(NAN_SUPPORTED_BAND_ID_6G)) != 0) &&
	...
	for (u4SlotIdx = 0; u4SlotIdx < NAN_TOTAL_SLOT_WINDOWS; u4SlotIdx++) {
		if (NAN_IS_AVAIL_MAP_SET(au4EmptyMap, u4SlotIdx)) {
			...
			nanSchedAddCrbToChnlList(prAdapter, &rSelChnlInfo,
				u4SlotIdx, 1, ENUM_TIME_BITMAP_CTRL_PERIOD_8192,
				FALSE, NULL);
		}
	}
			     ...
			     uint32_t u4SlotIdx)
{
	...
	for (u4AvailDbIdx = 0; u4AvailDbIdx < NAN_NUM_AVAIL_DB;
		...
		DBGLOG(NAN, LOUD,
		       ...
		       u4SlotIdx, prLocalChnlInfo->rChannel.u4PrimaryChnl,
		       rRmtChnlInfo.rChannel.u4PrimaryChnl);
		if (rRmtChnlInfo.rChannel.u4PrimaryChnl == 0 &&
		    !fgChkRmtCondSlot) {
			       ...
			       uint8_t fgChkRmtCondSlot)
{
	...
	for (u4DwIdx = 0; u4DwIdx < NAN_TOTAL_DW; u4DwIdx++) { /* 0..15 */
		rStatus = updateDWIntervalTimeline(prAdapter, u4DefCrbNum,
						   fgChkRmtCondSlot,
						   fgPeerAvailMapValid,
						   prSchedTimeline,
						   prNegoCtrl, u4DwIdx);
...
/* potential channel */
	...
	if (prAdapter->rWifiVar.ucNanBandChnlType ==
		...
		*pucPos = ((prScheduler->u4NumOfPotentialChnlList
		...
		pucPos++;
		for (u4Idx = 0;
			u4Idx < prScheduler->u4NumOfPotentialChnlList;
			u4Idx++) {
			pucPotentialChnls =
				(uint8_t *)&
				prScheduler->arPotentialChnlList[u4Idx];
...
nanSchedGetAvailabilityAttr(struct ADAPTER *prAdapter,
	...
	for (u4EntryIdx = 0, prChnlTimeline = &prNanTimelineMgmt->arChnlList[0];
		...
		if (IS_6G_OP_CLASS(prChnlInfo->rChannel.u4OperatingClass)) {
			DBGLOG(NAN, INFO,
		...
		nanParserGenBandChnlEntryListField(prAdapter, prChnlInfo,
...
nanSchedCmdUpdatePotentialChnlList(struct ADAPTER *prAdapter) {
	...
	if (prNanScheduler->fgEn6g) {
		...
		eBwMap = prAdapter->rWifiVar.ucNan6gBandwidth;
		u4Idx = 0;
		for (pr6gPotentialChnlMap = g_ar6gPotentialChnlMap;
	...
	if (rFixChnl.rChannel.u4PrimaryChnl != 0) {
		...
		prCmdUpdatePontentialChnlList->arChnlList[0].u2ChnlBitmap =
				 ...
				 uint32_t *pu4SlotBitmap)
{
	...
	uint32_t band_6G_slots = 0x00FF00FF;
	...
	*pu4SlotBitmap = 0;
	...
	for (i = 0; i < prAdapter->ucSwBssIdNum; i++) {
		...
		if (IS_BSS_AIS(prBssInfo) &&
			...
			*prChnl = nanRegGenNanChnlInfoByPriChannel(
			...
			if (prChnl->u4Type == NAN_BAND_CH_ENTRY_LIST_TYPE_BAND)
				DBGLOG(NAN, INFO,
				       "prBssInfo->eBand=%u, bw=%u, prChnl.type=%u, prChnl.band=%u\n",
				       prBssInfo->eBand, u4Bw, prChnl->u4Type,
				       prChnl->rBand.u4BandIdMask);
			else /* NAN_BAND_CH_ENTRY_LIST_TYPE_CHNL */
				DBGLOG(NAN, INFO,
				       "prBssInfo->eBand=%u, bw=%u, prChnl.type=%u, oc=%u, primaryChnl=%u, auxChnl=%u\n",
				       prBssInfo->eBand, u4Bw, prChnl->u4Type,
	 ...
	 */
	if (!prAdapter->rWifiVar.fgDbDcModeEn) {
		/* skip slot 31 for 2.4G DW channel switch */
		band_2G4_slots = 0x00FF00FC;
		band_5G_slots =  0x00FF00FC;
		band_6G_slots = 0x00FF00FC;
	}
...
uint32_t nanSchedCommitNonNanChnlList(struct ADAPTER *prAdapter)
{
	...
	if (prAdapter->rWifiVar.fgDbDcModeEn) {
		...
		if (nanRegGetNanChnlBand
			(nanSchedGetFixedChnlInfo(prAdapter)) == BAND_2G4) {
		...
		for (u4DwIdx = 0; u4DwIdx < NAN_TOTAL_DW; u4DwIdx++) {
			...
			if (rCommitChnlInfo.rChannel.u4PrimaryChnl != 0) {
				...
				if (nanSchedChkConcurrOp(
					...
					ENUM_TIME_BITMAP_CTRL_PERIOD_8192,
					TRUE);
					/* AIS slots marked here */
					nanSchedAddCrbToChnlList(prAdapter,
					&rNonNanChnlInfo, u4SlotIdx, 1,
					ENUM_TIME_BITMAP_CTRL_PERIOD_8192,
					TRUE, NULL);
				}
			} else
