	...
	uint8_t *tmac_info)
{
	...
	DBGLOG(HAL, INFO, "\t\tTxByteCnt = %lu\n",
		((txd->u4DW0 & CONNAC5X_TX_DESC_TX_BYTE_COUNT_MASK) >>
		CONNAC5X_TX_DESC_TX_BYTE_COUNT_OFFSET));

	/* Ether Type Offset [22:16]  */
	DBGLOG(HAL, INFO, "\t\tEtherTypeOffset = %lu\n",
		((txd->u4DW0 & CONNAC5X_TX_DESC_ETHER_TYPE_OFFSET_MASK) >>
		CONNAC5X_TX_DESC_ETHER_TYPE_OFFSET_OFFSET));

	/* PKT_FT: Packet Format [24:23] */
	DBGLOG(HAL, INFO, "\t\tpkt_ft = %lu(%s)\n",
	((txd->u4DW0 & CONNAC5X_TX_DESC_PACKET_FORMAT_MASK) >>
		CONNAC5X_TX_DESC_PACKET_FORMAT_OFFSET),
	pkt_ft_str[((txd->u4DW0 & CONNAC5X_TX_DESC_PACKET_FORMAT_MASK) >>
		CONNAC5X_TX_DESC_PACKET_FORMAT_OFFSET)]);

	/* Q_IDX [31:25]  */
	DBGLOG(HAL, INFO, "\t\tQueID =0x%lx\n",
		((txd->u4DW0 & CONNAC5X_TX_DESC_QUEUE_INDEX_MASK) >>
	...
	DBGLOG(HAL, INFO, "\t\tMLDID = %lu\n",
		((txd->u4DW1 & CONNAC5X_TX_DESC_MLD_ID_MASK) >>
		CONNAC5X_TX_DESC_MLD_ID_OFFSET));

	/* TGID [13:12] */
	DBGLOG(HAL, INFO, "\t\tTGID = %lu\n",
		((txd->u4DW1 & CONNAC5X_TX_DESC_TGID_MASK) >>
		CONNAC5X_TX_DESC_TGID_OFFSET));

	/* HF: Header Format [15:14] */
	DBGLOG(HAL, INFO, "\t\tHdrFmt = %lu(%s)\n",
	((txd->u4DW1 & CONNAC5X_TX_DESC_HEADER_FORMAT_MASK) >>
		CONNAC5X_TX_DESC_HEADER_FORMAT_OFFSET),
	hdr_fmt_str[((txd->u4DW1 & CONNAC5X_TX_DESC_HEADER_FORMAT_MASK) >>
		CONNAC5X_TX_DESC_HEADER_FORMAT_OFFSET)]);

	switch ((txd->u4DW1 & CONNAC5X_TX_DESC_HEADER_FORMAT_MASK) >>
		CONNAC5X_TX_DESC_HEADER_FORMAT_OFFSET) {
	...
	DBGLOG(HAL, INFO, "\t\tTID MGMT TYPE = %lu\n",
		((txd->u4DW1 & CONNAC5X_TX_DESC_TID_MGMT_TYPE_MASK) >>
		CONNAC5X_TX_DESC_TID_MGMT_TYPE_OFFSET));

	/* OM [30:25] */
	DBGLOG(HAL, INFO, "\t\town_mac = %lu\n",
		((txd->u4DW1 & CONNAC5X_TX_DESC_OWN_MAC_MASK) >>
		CONNAC5X_TX_DESC_OWN_MAC_OFFSET));

	/* FR [31] */
	DBGLOG(HAL, INFO, "\t\tFixedRate = %d\n",
		(txd->u4DW1 & CONNAC5X_TX_DESC_FIXED_RATE) ? 1 : 0);

	DBGLOG(HAL, INFO, "\tTMAC_TXD_2:\n");
	/* DW2 */
	/* Subtype [3:0] */
	DBGLOG(HAL, INFO, "\t\tsub_type = %lu\n",
		((txd->u4DW2 & CONNAC5X_TX_DESC_SUB_TYPE_MASK) >>
		CONNAC5X_TX_DESC_SUB_TYPE_OFFSET));

	/* Type[5:4] */
	DBGLOG(HAL, INFO, "\t\tfrm_type = %lu\n",
		((txd->u4DW2 & CONNAC5X_TX_DESC_TYPE_MASK) >>
		CONNAC5X_TX_DESC_TYPE_OFFSET));

	/* Beamform Type[7:6] */
	DBGLOG(HAL, INFO, "\t\tBeanform_type = %lu\n",
		((txd->u4DW2 & CONNAC5X_TX_DESC_BEAMFORM_TYPE_MASK) >>
	...
	DBGLOG(HAL, INFO, "\t\tHeader_padding = %lu\n",
		((txd->u4DW2 & CONNAC5X_TX_DESC_HEADER_PADDING) >>
		CONNAC5X_TX_DESC_HEADER_PADDING_OFFSET));

	/* DU [12] */
	DBGLOG(HAL, INFO, "\t\tDuration = %d\n",
	((txd->u4DW2 & CONNAC5X_TX_DESC_DURATION_FIELD_CONTROL) ? 1 : 0));

	/* HE [13] */
	DBGLOG(HAL, INFO, "\t\tHE(HTC Exist) = %d\n",
		((txd->u4DW2 & CONNAC5X_TX_DESC_HTC_EXISTS) ? 1 : 0));

	/* FRAG [15:14] */
	DBGLOG(HAL, INFO, "\t\tFRAG = %lu\n",
		((txd->u4DW2 & CONNAC5X_TX_DESC_FRAGMENT_MASK) >>
		CONNAC5X_TX_DESC_FRAGMENT_OFFSET));

	/* Remaining Life Time [25:16]*/
	DBGLOG(HAL, INFO, "\t\tReamingLife/MaxTx time = %lu\n",
		((txd->u4DW2 & CONNAC5X_TX_DESC_REMAINING_MAX_TX_TIME_MASK) >>
		CONNAC5X_TX_DESC_REMAINING_MAX_TX_TIME_OFFSET));

	/* Power Offset [31:26] */
	DBGLOG(HAL, INFO, "\t\tpwr_offset = %lu\n",
		((txd->u4DW2 & CONNAC5X_TX_DESC_POWER_OFFSET_MASK) >>
		CONNAC5X_TX_DESC_POWER_OFFSET_OFFSET));

	DBGLOG(HAL, INFO, "\tTMAC_TXD_3:\n");
	/* DW3 */
	/* NA [0] */
	DBGLOG(HAL, INFO, "\t\tNoAck = %d\n",
		(txd->u4DW3 & CONNAC5X_TX_DESC_NO_ACK) ? 1 : 0);

	/* PF [1] */
	DBGLOG(HAL, INFO, "\t\tPF = %d\n",
		(txd->u4DW3 & CONNAC5X_TX_DESC_PROTECTED_FRAME) ? 1 : 0);

	/* EMRD [2] */
	DBGLOG(HAL, INFO, "\t\tEMRD = %d\n",
		(txd->u4DW3 & CONNAC5X_TX_DESC_EXTEND_MORE_DATA) ? 1 : 0);

	/* EEOSP [3] */
	DBGLOG(HAL, INFO, "\t\tEEOSP = %d\n",
		(txd->u4DW3 & CONNAC5X_TX_DESC_EXTEND_EOSP) ? 1 : 0);
	...
	DBGLOG(HAL, INFO, "\t\ttx_cnt = %lu\n",
		((txd->u4DW3 & CONNAC5X_TX_DESC_TX_COUNT_MASK) >>
		CONNAC5X_TX_DESC_TX_COUNT_OFFSET));

	/* Remaining TX Count [15:11] */
	DBGLOG(HAL, INFO, "\t\tremain_tx_cnt = %lu\n",
		((txd->u4DW3 & CONNAC5X_TX_DESC_REMAINING_TX_COUNT_MASK) >>
		CONNAC5X_TX_DESC_REMAINING_TX_COUNT_OFFSET));

	/* SN [27:16] */
	DBGLOG(HAL, INFO, "\t\tsn = %lu\n",
		((txd->u4DW3 & CONNAC5X_TX_DESC_SEQUENCE_NUMBER_MASK) >>
		CONNAC5X_TX_DESC_SEQUENCE_NUMBER_MASK_OFFSET));

	/* BA_DIS [28] */
	DBGLOG(HAL, INFO, "\t\tba dis = %d\n",
		(txd->u4DW3 & CONNAC5X_TX_DESC_BA_DISABLE) ? 1 : 0);

	/* Power Management [29] */
	DBGLOG(HAL, INFO, "\t\tpwr_mgmt = 0x%x\n",
	(txd->u4DW3 & CONNAC5X_TX_DESC_POWER_MANAGEMENT_CONTROL) ? 1 : 0);

	/* PN_VLD [30] */
	DBGLOG(HAL, INFO, "\t\tpn_vld = %d\n",
		(txd->u4DW3 & CONNAC5X_TX_DESC_PN_IS_VALID) ? 1 : 0);

	/* SN_VLD [31] */
	DBGLOG(HAL, INFO, "\t\tsn_vld = %d\n",
		(txd->u4DW3 & CONNAC5X_TX_DESC_SN_IS_VALID) ? 1 : 0);
	...
	DBGLOG(HAL, INFO, "\t\tTX_packet_source = %lu\n",
		(txd->u4DW6 & CONNAC5X_TX_DESC_TX_PACKET_SOURCE_MASK) >>
	...
	DBGLOG(HAL, INFO, "\t\ttxd len= %lu\n",
		((txd->u4DW7 & CONNAC5X_TX_DESC_TXD_LENGTH_MASK) >>
		CONNAC5X_TX_DESC_TXD_LENGTH_OFFSET));
}

static void connac5x_event_dump_txd_mem(
	struct ADAPTER *prAdapter,
...

{
	...
	connac5x_dump_tmac_info(prAdapter, &data[0]);
}
		...
		uint8_t ucStaIdx)
{
	...
	/* P-RXV0 */
	rate = CONNAC5X_GET_RX_RATE_FROM_ADAPTER(prAdapter, ucStaIdx);
	nsts = CONNAC5X_GET_RX_NSTS_FROM_ADAPTER(prAdapter, ucStaIdx);
	ldpc = CONNAC5X_GET_RX_LDPC_FROM_ADAPTER(prAdapter, ucStaIdx);
	frmode = CONNAC5X_GET_RX_FR_MODE_FROM_ADAPTER(prAdapter, ucStaIdx);
	sgi = CONNAC5X_GET_RX_GI_FROM_ADAPTER(prAdapter, ucStaIdx);
	stbc = CONNAC5X_GET_RX_STBC_ADAPTER(prAdapter, ucStaIdx);
	txmode = CONNAC5X_GET_RX_MODE_ADAPTER(prAdapter, ucStaIdx);
	mu = CONNAC5X_GET_RX_MU_ADAPTER(prAdapter, ucStaIdx);
	dcm = CONNAC5X_GET_RX_DCM_ADAPTER(prAdapter, ucStaIdx);
		...
		uint8_t ucStaIdx)
{
	...
	i4RSSI0 = RCPI_TO_dBm(CONNAC5X_GET_RCPI0_FROM_ADAPTER(
			prAdapter, ucStaIdx));
	i4RSSI1 = RCPI_TO_dBm(CONNAC5X_GET_RCPI1_FROM_ADAPTER(
			prAdapter, ucStaIdx));

	if (prAdapter->rWifiVar.ucNSS > 2) {
		i4RSSI2 = RCPI_TO_dBm(CONNAC5X_GET_RCPI2_FROM_ADAPTER(
				prAdapter, ucStaIdx));
		i4RSSI3 = RCPI_TO_dBm(CONNAC5X_GET_RCPI3_FROM_ADAPTER(
				prAdapter, ucStaIdx));
...
int32_t connac5xGetTxRateInfo(char *pcCommand, int i4TotalLen,
			u_int8_t fgDumpAll,
			...
			struct PARAM_GET_STA_STATISTICS *prQueryStaStatistics)
{
	...
	for (i = 0; i < AUTO_RATE_NUM; i++) {
		...
		sgi = connac5x_wtbl_get_sgi_info(pWtbl);
		dcm = HW_TX_RATE_TO_DCM(arTxRate[i]);
		ersu106t = HW_TX_RATE_TO_106T(arTxRate[i]);

		if (dcm)
			rate = CONNAC5X_HW_TX_RATE_UNMASK_DCM(rate);
		if (ersu106t)
			rate = CONNAC5X_HW_TX_RATE_UNMASK_106T(rate);
		...
		if (prQueryStaStatistics->ucSkipAr) {
			i4BytesWritten += kalScnprintf(
				...
				stbc ? ", STBC, " : ", ",
				connac5x_wtbl_get_ldpc_info(txmode, pWtbl)
					== 0 ? "BCC" : "LDPC");
		} else {
			...
			if (prQueryStaStatistics->aucArRatePer[
				...
				i4BytesWritten += kalScnprintf(
					...
					txmode < ENUM_TX_MODE_NUM ?
					    HW_TX_MODE_STR[txmode] : "N/A",
					stbc ? ", STBC, " : ", ",
					((connac5x_wtbl_get_ldpc_info(txmode,
						pWtbl) == 0)
...
int32_t connac5x_get_tx_info_from_txv(char *pcCommand,
	int i4TotalLen,
	struct TX_VECTOR_BBP_LATCH *prTxV)
{
	...
	if (dcm)
		rate = CONNAC5X_TXV_GET_TX_RATE_UNMASK_DCM(rate);
	if (ersu106t)
		rate = CONNAC5X_TXV_GET_TX_RATE_UNMASK_106T(rate);
		...
		uint32_t u4StatGroup)
{
	...
	if (u4StatGroup & 0x0020) {
		...
		/* Rate1~Rate8 */
		i4BytesWritten += connac5xGetTxRateInfo(
			pcCommand + i4BytesWritten, i4TotalLen - i4BytesWritten,
	...
	uint32_t u4DmaNum)
{
	...
	u4hostBaseCrAddr = WFDMA_TYPE_HOST ?
		CONNAC5X_MCU_INT_CONN_HIF_WRAP :
	...
	u4DmaCfgCrAddr = CONNAC5X_WPDMA_INT_STA(u4hostBaseCrAddr);
	...
	u4DmaCfgCrAddr = CONNAC5X_WPDMA_INT_MASK(u4hostBaseCrAddr);
	...
	uint32_t u4DmaNum)
{
	...
	u4DmaCfgCrAddr = CONNAC5X_WPDMA_GLO_CFG(u4hostBaseCrAddr);
...
void connac5x_show_wfdma_desc(struct ADAPTER *prAdapter)
{
...
static void connac5xDumpPPDebugCr(struct ADAPTER *prAdapter)
{
	struct BUS_INFO *prBusInfo;
	...
	uint32_t u4DmaNum)
{
	...
	DBGLOG(HAL, INFO, "==============================\n");
	connac5x_show_wfdma_interrupt_info(
		prAdapter, enum_wfdma_type, u4DmaNum);
	connac5x_show_wfdma_glo_info(
		prAdapter, enum_wfdma_type, u4DmaNum);
	connac5x_show_wfdma_ring_info(
...
void connac5x_show_wfdma_info(struct ADAPTER *prAdapter)
{
	...
	connac5x_show_wfdma_info_by_type(prAdapter, WFDMA_TYPE_HOST, u4DmaNum);
	connac5x_show_wfdma_dbg_flag_log(prAdapter, WFDMA_TYPE_HOST, u4DmaNum);

	if (prBusInfo->wfmda_wm_tx_group && prBusInfo->wfmda_wm_rx_group) {
		connac5x_show_wfdma_info_by_type(
			prAdapter, WFDMA_TYPE_WM, u4DmaNum);
		connac5x_show_wfdma_dbg_flag_log(
			prAdapter, WFDMA_TYPE_WM, u4DmaNum);
	}
...
#if CFG_MTK_WIFI_WFDMA_WB
	connac5x_show_wfdma_wb_info(prAdapter);
#endif /* CFG_MTK_WIFI_WFDMA_WB */

#if (CFG_SUPPORT_HOST_OFFLOAD == 1)
	if (IS_FEATURE_ENABLED(prWifiVar->fgEnableMawdTx))
		connac5x_show_mawd_info(prAdapter);
...
static void asicConnac5xDmashdlGetPktMaxPage(struct ADAPTER *prAdapter)
{
	struct BUS_INFO *prBusInfo;
...
static void asicConnac5xDmashdlGetRefill(struct ADAPTER *prAdapter)
{
	struct BUS_INFO *prBusInfo;
	struct DMASHDL_CFG *prCfg;
	uint32_t u4Val = 0;
...
static void asicConnac5xDmashdlLiteGetMainInfo(struct ADAPTER *prAdapter)
{
	struct BUS_INFO *prBusInfo;
	struct DMASHDL_CFG *prCfg;
...
static void asicConnac5xDmashdlGetGroupControl(struct ADAPTER *prAdapter,
					       uint8_t ucGroup)
{
	struct BUS_INFO *prBusInfo;
	struct DMASHDL_CFG *prCfg;
...
static uint32_t asicConnac5xDmashdlGetRsvCount(struct ADAPTER *prAdapter,
					       uint8_t ucGroup)
{
	struct BUS_INFO *prBusInfo;
	struct DMASHDL_CFG *prCfg;
...
static uint32_t asicConnac5xDmashdlGetSrcCount(struct ADAPTER *prAdapter,
					       uint8_t ucGroup)
{
	struct BUS_INFO *prBusInfo;
	struct DMASHDL_CFG *prCfg;
...
static uint32_t asicConnac5xDmashdlLiteGetSrcCount(
	struct ADAPTER *prAdapter, uint8_t ucGroup)
{
	...
	src_cnt = asicConnac5xDmashdlGetSrcCount(prAdapter, ucGroup);
...
static void asicConnac5xDmashdlGetPKTCount(struct ADAPTER *prAdapter,
					   uint8_t ucGroup)
{
	struct BUS_INFO *prBusInfo;
	struct DMASHDL_CFG *prCfg;
...
void connac5x_show_dmashdl_info(struct ADAPTER *prAdapter)
{
	...
	asicConnac5xDmashdlGetRefill(prAdapter);
	asicConnac5xDmashdlGetPktMaxPage(prAdapter);
	...
	for (idx = 0; idx <= ENUM_DMASHDL_GROUP_14; idx++) {
		...
		asicConnac5xDmashdlGetGroupControl(prAdapter, idx);
		rsv_cnt = asicConnac5xDmashdlGetRsvCount(prAdapter, idx);
		src_cnt = asicConnac5xDmashdlGetSrcCount(prAdapter, idx);
		asicConnac5xDmashdlGetPKTCount(prAdapter, idx);
	...
	DBGLOG(HAL, INFO, "Group %d info:\n", idx);
	asicConnac5xDmashdlGetGroupControl(prAdapter, idx);
	asicConnac5xDmashdlGetRsvCount(prAdapter, idx);
	asicConnac5xDmashdlGetSrcCount(prAdapter, idx);
	asicConnac5xDmashdlGetPKTCount(prAdapter, idx);
...
void connac5x_show_dmashdl_lite_info(struct ADAPTER *prAdapter)
{
	...
	asicConnac5xDmashdlLiteGetMainInfo(prAdapter);
	...
	for (idx = 0; idx <= ENUM_DMASHDL_GROUP_14; idx++) {
		...
		DBGLOG(HAL, INFO, "Group %d info:\n", idx);
		asicConnac5xDmashdlGetGroupControl(prAdapter, idx);
		src_cnt = asicConnac5xDmashdlLiteGetSrcCount(prAdapter, idx);
		total_src_cnt += src_cnt;
	}
	...
	for (idx = ENUM_DMASHDL_GROUP_15;
		...
		DBGLOG(HAL, INFO, "Group %d info:\n", idx);
		asicConnac5xDmashdlGetGroupControl(prAdapter, idx);
		asicConnac5xDmashdlLiteGetSrcCount(prAdapter, idx);
	}
...
int connac5x_get_rx_rate_info(const uint32_t *prRxV,
		struct RxRateInfo *prRxRateInfo)
{
	...
	/* P-RXV1 */
	rate = (prRxV[0] & CONNAC5X_RX_VT_RX_RATE_MASK)
				>> CONNAC5X_RX_VT_RX_RATE_OFFSET;
	nsts = (prRxV[0] & CONNAC5X_RX_VT_NSTS_MASK)
				>> CONNAC5X_RX_VT_NSTS_OFFSET;
	frmode = (prRxV[2] & CONNAC5X_RX_VT_FR_MODE_MASK)
					>> CONNAC5X_RX_VT_FR_MODE_OFFSET;
	sgi = (prRxV[2] & CONNAC5X_RX_VT_GI_MASK)
					>> CONNAC5X_RX_VT_GI_OFFSET;
	rxmode = (prRxV[2] & CONNAC5X_RX_VT_RX_MODE_MASK)
					>> CONNAC5X_RX_VT_RX_MODE_OFFSET;
	stbc = (prRxV[2] & CONNAC5X_RX_VT_STBC_MASK)
				>> CONNAC5X_RX_VT_STBC_OFFSET;
	mu = prRxV[0] & CONNAC5X_RX_VT_MU_MASK;
